...
- **`packages/cadjs/`** — UI-framework-agnostic JS for CAD parsing/rendering/topology consumed by the viewer. Source of truth. `viewer/packages/cadjs/` is a **generated mirror** — never hand-edit.
- **`packages/cadpy/`** — Python build123d + cadquery-ocp pipeline that turns a `gen_step()` project into `.step` + `.glb` + `.topology.json` + `.step.json` (and optional `.stl` / `.3mf`). Source of truth. Vendored copies under skill runtimes (e.g. `skills/cadcode/scripts/packages/cadpy/`) are **generated mirrors** — never hand-edit.
- **`skills/`** — Claude Code skills bundled with the app (`cadcode`, `cad-viewer`, `gcode`, `bambu-labs`, `step-parts`). Invoked by the `claude` CLI subprocess, not by the Rust driver directly. Each skill is self-contained at runtime — no cross-skill imports, no imports from repo root; shared helpers are vendored in via `scripts/build/build-skill-runtimes.sh`.
...
1. `gen_step()` CadQuery/build123d contract — what a project's `main.py` may return, the artifacts written, topology-ID stability rules. Both libraries are dispatched in one entry point via duck-typing on `.wrapped` / `TopoDS_Shape`; do not add isinstance checks.
...
3. Skill stdout + artifact contract — `cadcode` skill must print exactly one JSON line to stdout; Rust driver mtime-snapshots `.step .stp .stl .3mf .glb .gcode .png .py .json` (case-sensitive, recursive) to emit `artifact_changed` events.
...
- **Topology IDs** (`@cad[…#f3]`) come from `TopExp.MapShapes_s(shape.wrapped, TopAbs_*)` — never from CadQuery tags or build123d selectors. A build123d `Box(20,20,5)` and `cq.Workplane().box(20,20,5)` must yield identical face-ID ordinals.
