...
test("loadRenderStl resolves on the main thread without using the worker", async (t) => {
  // Regression for the forever-spinner bug: STL used to be hard-wired to a
  // Web Worker that fetched the pandaasset:// scheme and never settled,
  // hanging the load. With the worker gated behind preferWorker (default
  // false), the load must resolve on the main thread. We install a Worker
  // that NEVER replies — if loadRenderStl touched it, this test would hang.
  const asciiStl = [
    "solid x",
    "facet normal 0 0 1",
    " outer loop",
  ...
  const stlBytes = new TextEncoder().encode(asciiStl);
  ...
  globalThis.fetch = async () => {
    fetched = true;
    return new Response(stlBytes, { status: 200 });
  };
  globalThis.Worker = class HangingWorker {
    addEventListener() {}
    removeEventListener() {}
    postMessage() {} // never replies → would hang if used
    terminate() {}
  };
  ...
  // Unique URL so we don't hit the module-level mesh cache.
  const url = "pandaasset://localhost/regression-main-thread.stl";
  const result = await loadRenderStl(url);

  assert.equal(fetched, true, "expected a main-thread fetch of the STL bytes");
  assert.equal(result.sourceFormat, "stl");
  assert.ok(result.vertices.length > 0, "expected parsed STL vertices");
});
