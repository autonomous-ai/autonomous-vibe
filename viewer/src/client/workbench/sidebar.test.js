...
test("sidebarLabelForEntry shows the bare part stem without a file extension", () => {
  const entry = {
    file: "sample_assembly.step",
    kind: "assembly",
    source: { format: "step", path: "assemblies/sample_assembly.step" }
  ...
  // Consumers see "parts", not files — no .step/.stl/.glb suffix in the rail.
  assert.equal(sidebarLabelForEntry(entry), "sample_assembly");
  assert.equal(
    sidebarLabelForEntry({
      file: "parts/bracket.stl",
      kind: "stl",
      source: { format: "stl", path: "parts/bracket.stl" }
    }),
    "bracket"
  );
});
...
// --- Change 3: auto-select newest printable part after a build -------------
// Mirrors the matching CadWorkspace uses: a build emits artifact_changed for a
// printable file (.stl/.3mf/.glb); we resolve it to its catalog entry by the
// extension-stripped path (cadPathForEntry) and prefer the printable variant
// over the archival raw STEP (which the rail hides).
test("Change 3: a changed printable artifact resolves to its printable catalog entry, not the raw STEP", () => {
  const artifactFile = "parts/bracket.stl";
  const stem = artifactFile.replace(/\.(stl|3mf|glb)$/i, "");
  assert.equal(stem, "parts/bracket");

  const entries = [
    // Archival B-rep — hidden from the rail, must NOT be auto-selected.
    { file: "parts/bracket.step", kind: "part", source: { format: "step", path: "parts/bracket.step" } },
    // Printable mesh sibling — the thing the user should land on.
    { file: "parts/bracket.glb", kind: "glb", source: { format: "glb", path: "parts/bracket.glb" } },
  ];

  const match = entries.find(
    (entry) => isPrintableModelEntry(entry) && cadPathForEntry(entry) === stem
  );
  assert.ok(match, "found a printable entry for the changed artifact");
